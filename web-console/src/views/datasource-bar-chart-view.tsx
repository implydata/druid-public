...
import './datasource-bar-chart-view.scss';

interface DatasourceBarChartViewProps extends React.Props<any> {

}

interface DatasourceBarChartViewState {
...
export class DatasourceBarChartView extends React.Component<DatasourceBarChartViewProps, DatasourceBarChartViewState> {
  ...
  private chartMargin = { top: 20, right: 20, bottom: 20, left: 20 };

  constructor(props: DatasourceBarChartViewProps) {
    super(props);
    this.state = {
      data: {},
      datasources: [],
  ...
  componentDidUpdate(prevProps: DatasourceBarChartViewProps, prevState: DatasourceBarChartViewState): void {
  ...
  private calculateStackedData(data: Record<string, any>): Record<string, BarUnitData[]> {
    const { datasources, chartWidth, activeDataType } = this.state;
  ...
  private calculateScales(): BarChartScales | null {
    const {data, activeDataType, activeDatasource, singleDatasourceData, chartWidth, chartHeight } = this.state;
    ...
    const xScale: AxisScale<Date> = d3.scaleTime().domain(xDomain).range([0, chartWidth - 40]);
    const yScale: AxisScale<number> = d3.scaleLinear().rangeRound([chartHeight - 40, 0]).domain(yDomain);
    return {
      xScale,
      yScale
    };
  ...
  renderStackedBarChart() {
    const { loading, chartHeight, chartWidth, dataToRender, activeDataType, error, xScale, yScale, data, activeDatasource } = this.state;
    if (loading) {
      return <div className={'loading-bar-chart'}>
        <Loader
          loadingText={''}
          loading={loading}
        />
  ...
  render() {
    ...
    return <div className={'datasource-bar-chart-view app-view'}>
      <ViewControlBar label="Segment timeline">
        <Button
          text={'View all'}
          onClick={() => this.setState({activeDatasource: null})}
          disabled={activeDatasource === null}
        />
        ...
        <div>
          ...
          >
            <option value="countData">Count</option>
            <option value="sizeData">Size</option>
          </HTMLSelect>
        </div>
