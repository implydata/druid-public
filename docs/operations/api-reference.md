...
Retrieve a [task completion report](../ingestion/reports.md) for a task. Only works for completed tasks.
...
Retrieve a live row stats report from a Peon. See [task reports](../ingestion/reports.md) for more details.

* `/druid/worker/v1/chat/{taskId}/unparseableEvents`

Retrieve an unparseable events report from a Peon. See [task reports](../ingestion/reports.md) for more details.
