...
To use this extension, make sure to [include](../../operations/including-extensions.md) `druid-orc-extensions`.
...
`orc` supports auto field discovery and flattening, if specified with a [flattenSpec](../../ingestion/flatten-json.md). 
...
[`flattenSpec` expressions](../../ingestion/flatten-json.md). For example, `"typeString":"struct<time:string,name:string>"`
...
* The 'contrib' extension supported a `mapFieldNameFormat` property, which provided a way to specify a dimension to
 ...
 [`flattenSpec` expressions](../../ingestion/flatten-json.md). For example: `"mapFieldNameFormat": "<PARENT>_<CHILD>"`
