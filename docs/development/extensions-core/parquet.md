...
Note: `druid-parquet-extensions` depends on the `druid-avro-extensions` module, so be sure to
 [include  both](../../operations/including-extensions.md).
...
[flattenSpec](../../ingestion/flatten-json.md) with `parquet` or `avro` as the format. Parquet nested list and map 
...
For additional details see [hadoop ingestion](../../ingestion/hadoop.md) and [general ingestion spec](../../ingestion/ingestion-spec.md) documentation.